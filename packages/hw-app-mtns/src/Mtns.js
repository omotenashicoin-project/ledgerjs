 ...
 *
 * @example
 * import Mtns from "@mtns/hw-app-mtns";
 * const mtns = new Mtns(transport)
 */
export default class Mtns {
  transport: Transport<*>;

  constructor(transport: Transport<*>, scrambleKey: string = "MTNS") {
  ...
  getWalletPublicKey_private(
    ...
    if (!(format in addressFormatMap)) {
      throw new Error("mtns.getWalletPublicKey invalid format=" + format);
    }
   ...
   *
   * @example
   * mtns.getWalletPublicKey("44'/341'/0'/0/0").then(o => o.bitcoinAddress)
   * mtns.getWalletPublicKey("49'/0'/0'/0/0", { format: "p2sh" }).then(o => o.bitcoinAddress)
   */
  getWalletPublicKey(
    path: string,
    opts?:
    ...
    if (arguments.length > 2 || typeof opts === "boolean") {
      console.warn(
        "mtns.getWalletPublicKey deprecated signature used. Please switch to getWalletPublicKey(path, { format, verify })"
      );
      options = {
        verify: !!opts,
   ...
   * @example
   mtns.signMessageNew_async("44'/60'/0'/0'/0", Buffer.from("test").toString("hex")).then(function(result) {
     var v = result['v'] + 27 + 4;
   ...
   * @example
mtns.createPaymentTransactionNew(
   [ [tx1, 1] ],
   ["0'/0/0"],
   undefined,
...
mtns.signP2SHTransaction(
 ...
 "01905f0100000000001976a91472a5d75c8d2d0565b656a5232703b167d50d5a2b88ac"
   ...
   * @example
const tx1 = mtns.splitTransaction("");
   */
  splitTransaction(
  ...
  @example
const tx1 = mtns.splitTransaction("");
const outputScript = mtns.serializeTransactionOutputs(tx1).toString('hex');
  */
